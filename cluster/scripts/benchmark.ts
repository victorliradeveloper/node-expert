 ...
 * A resposta inclui o workerId (PID do worker process que atendeu a requisição),
 * mostrando visualmente como o cluster distribui carga entre os processos.
 *
 * Uso: ts-node scripts/benchmark.ts [concorrentes] [arquivo]
 * Exemplo: ts-node scripts/benchmark.ts 8 test-files/large.csv
 */
...
function upload(filePath: string): Promise<{ jobId: string; workerId: number; ms: number }> {
      ...
      (res) => {
        ...
        res.on('end', () => {
          try {
            ...
            resolve({ jobId, workerId, ms: Date.now() - start });
          } catch {
...
async function pollResult(jobId: string): Promise<any> {
...
async function main() {
  ...
  uploads.forEach(({ jobId, workerId, ms }, i) => {
    console.log(`  Upload ${i + 1}: jobId=${jobId}  worker=${workerId}  resposta em ${ms}ms`);
  });

  // Sumariza distribuição por worker
  const byWorker = uploads.reduce((acc, { workerId }) => {
    acc[workerId] = (acc[workerId] ?? 0) + 1;
    return acc;
  }, {} as Record<number, number>);
  console.log('\nDistribuição por worker process:');
  for (const [pid, count] of Object.entries(byWorker)) {
    console.log(`  PID ${pid}: ${count} upload(s)`);
  }
  ...
  results.forEach((r: any, i) => {
    console.log(`  Job ${i + 1}: ${r.status} — ${r.processed} linhas em ${r.elapsedMs}ms (worker ${r.workerId})`);
  });
}
